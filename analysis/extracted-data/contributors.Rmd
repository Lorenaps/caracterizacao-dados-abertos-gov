...
## Qual a frequência de contribuições?
```{r}

contributors %>% 
  select(numero_contribuicoes = count) %>% 
  group_by(numero_contribuicoes) %>% 
  summarise(frequencia = n()) %>%
  ggplot(mapping = aes(x = numero_contribuicoes, y = frequencia, label=frequencia)) + 
                geom_line() + 
                xlab("Quantidade de contribuições") + 
                ylab("Frequencia") + 
                geom_label() + 
...
#### Por hora
Podemos observar que a rede de contribuições é formada por muitos grupos separados. 
Temos em maioria repositórios com 2 contribuintes. E poucos repositórios que compartilham de uma mesma pessoa contribuidora.